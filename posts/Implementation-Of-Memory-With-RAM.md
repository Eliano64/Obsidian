---
title: Implementation Of Memory With RAM
tags:
  - Memory
categories:
...
![1000](../assets/main-memory-word-expansion-wiring.svg)
...
用多片存储芯片组成主存时，每次访问要完成两级选择：先选中一片或一组芯片，再选择芯片内部的存储单元。

低位地址线用于芯片内部寻址；剩余的高位地址线用于产生**片选信号**，决定由哪片或哪组芯片响应。

高位地址线产生片选信号有三种方法：

| 方法 | 高位地址线的使用方式 | 结果 |
| --- | --- | --- |
| 全译码法 | 所有未用于片内寻址的高位地址线都参加译码 | 每片芯片只对应一个地址区间，不会出现地址重叠；译码电路较复杂 |
| 部分译码法 | 只让一部分高位地址线参加译码，其余高位地址线不参与 | 电路较简单，但同一存储单元会对应多个系统地址 |
| 线选法 | 不使用译码器，用一根高位地址线直接选择一片或一组芯片 | 电路最简单，但必须避免多根片选线同时有效，地址空间利用率较低 |

下面统一假设系统有 16 位地址，使用容量为 `4K` 的芯片。芯片内部需要 `A0-A11` 共 12 位地址，因此 `A15-A12` 用于片选。

## 全译码法

全译码法把 `A15-A12` 全部送入译码器，每一种高四位组合只选中一片或一组芯片。

例如规定 `A15A14A13A12=1000` 时选中某芯片，则只有 `8000H-8FFFH` 会访问该芯片：高四位固定为 `1000`，低 12 位从全 0 变化到全 1。由于全部高位都参与译码，该芯片不会在其他地址区间再次出现。

## 部分译码法

部分译码，即只使用高位地址的一部分参与译码，而另一部分与译码电路无关。

会出现一个物理存储单元对应多个地址的情况，即**地址重叠** 。 

此时系统地址组成：

|     | 参与译码的高位 | 不参与片选译码的高位 | 芯片内部地址    |
| --- | ------- | ---------- | --------- |
| 取值  | 固定值     | 任意值        | 从全 0 到全 1 |
...
线选法不设置译码器，它把不同的高位地址线分别接到不同芯片的片选端。以片选高电平有效为例，可以用 `A12` 选择第一片、`A13` 选择第二片、`A14` 选择第三片、`A15` 选择第四片。

正常访问时应让四根片选线中只有一根为 1。例如用 `A12` 选择第一片时，高四位取 `0001`，其地址区间为 `1000H-1FFFH`

因此线选法虽然省去译码器，但可用的高位地址组合较少。

# SRAM 与 DRAM

SRAM 和 DRAM 都是半导体 RAM，但存储信息方式不同，导致速度、成本、刷新、用途都不同。

| 对比项    | SRAM                 | DRAM           |
| ------ | -------------------- | -------------- |
| 存储信息方式 | 双稳态触发器, 有 0/1 两种稳定状态 | 电容, 是否带电表示 0/1 |
...
> [!danger] SRAM 仍为易失性存储器
> SRAM 虽然不需要刷新，但这不意味着要保持数据可以不供电。
...
> [!info] 采用地址复用技术的 DRAM 芯片的引脚数
> DRAM 把行地址和列地址分两次送入同一组地址引脚。若行地址为 $n_r$ 位、列地址为 $n_c$ 位、存储字长为 $m$ 位，则主要引脚包括：
>
> | 引脚 | 数量 | 说明 |
> | --- | --- | --- |
> | 复用地址引脚 | $\max(n_r,n_c)$ | 先传行地址，再传列地址 |
> | 数据引脚 | $m$ | 传送一个存储字 |
> | `RAS`、`CAS` | 2 | 分别锁存并选通行地址、列地址 |
> | 读写控制 | 按芯片接口确定 | 常见为单个 `WE`，有的器件另设 `OE` |
> | 片选 | 按芯片接口确定 | 有的器件单独设置 `CS/CE` |
> | 电源、接地 | 通常 2 | `VCC` 与 `GND` |
>
> 为了减少地址引脚，行地址位数 $n_r$ 与列地址位数 $n_c$ 应尽量接近。
...
> [!example] 求刷新一次的平均时间间隔
> 若 DRAM 阵列有 128 行，刷新间隔为 2ms，刷新周期 0.5$\mu$s：
> 1. 采用**异步刷新**，则平均每隔 $\frac{2ms}{128} -0.5 \mu s=15.125\mu s$ 至少要刷新一行。
> 2. 采用**集中刷新**，则每隔 $2ms-0.5\mu s \cdot 128=1.921ms$ 开始集中地刷新 128 行。在这段时间内 DRAM 构成的存储器无法提供访存服务，即**死区**。
...
令 $m$ 为模块数，$T$ 为每个模块的存取周期，$T_A$为每个模块的存取时间。$r$ 为总线传输一次数据所需要的时间。

则模块间应间隔 $T_{w}=\max\left\{ r, \frac{T}{m} \right\}$ 时间启动。

这样，若采用 [[Bus-Performance#突发传输|突发传输]] ，且读数据总线事务过程包括传首地址与读命令、存储器准备数据、传数据，则读 $m$ 个字所需时间为：

1. 传首地址与读命令
2. 存储器准备数据：独立来看，每一个字都需要$T_{A}$，但由于是**流水线**方式，所以只有第一个字的$T_{A}$是必须要等的。后面每准备一个字实际多出来的时间是$T_{w}$，且与前一个字的传输可以同时进行。因为存储器准备数据并没有占用总线。
3. 传数据：等这个字准备好后，就可以传数据了。时间为$r$。由于与后一个字的准备同时进行，所以每传一个字实际多出来的时间是$T_{w}$。
### 同时启动

同时启动是一次向多个存储模块同时发出读写请求，让多个模块在同一个存取周期内并行工作。它适合把多个相邻地址的数据作为一批取出。

若有 $m$ 个模块，每个模块一次输出一个字，则一次同时启动最多可以得到 $m$ 个字。此时：
