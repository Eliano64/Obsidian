...
> [!warning] 不带权/带权图的邻接矩阵的矩阵单元的语义不同
> 在不带权邻接矩阵中，矩阵单元的语义是" $v_i$ 是否与 $v_j$存在弧 $<v_i,v_{j}>$ "。`1` 表示 `True`，`0` 表示 `False` 。由于主对角线上对应的是点到点本身，而在简单图的语境下一点到自身确实不会存在边相连，所以主对角线写为 `0` 。
>
> 在带权图的代价矩阵中，矩阵单元表示" 从 $v_i$ 到 $v_j$ 的代价 "。所以不直接到达的两个顶点对应的矩阵单元应写成 $\infty$，主对角线写成 `0` 。
