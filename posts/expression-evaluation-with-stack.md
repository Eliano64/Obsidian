...
1. 遇到操作数：直接加入后缀表达式。
2. 遇到左括号：入栈。
3. 遇到右括号：弹出栈顶，栈顶加入后缀表达式，一直这样操作直到弹出左括号；左右括号不进入后缀表达式。
4. 遇到运算符：弹出栈中优先级高于或等于当前运算符的运算符，并将被弹出的运算符加入后缀表达式，直到遇到左括号/栈空/栈顶优先级低于当前运算符。再把当前运算符入栈。
5. 扫描结束：弹出栈中剩余运算符并加入后缀表达式。

同级运算符按从左到右结合时，遇到当前运算符要先弹出栈内优先级相同的运算符，这就是左优先原则。
