...
十字链表用于存储**有向图**。它类似于将邻接表和逆邻接表合并在一起，以实现快速找到入边。
...
邻接多重表用于存储**无向图**。它利用无向图边$(i,j)=(j,i)$ 的特点，将原来用邻接表表示时的$<i,j>$和$<j,i>$的两个边结点整合在了一起，以实现减少存储冗余。

它的核心规则是：一个边结点同时指向有顶点 `i` 的下一条边和有有顶点 `j` 的下一条边。
...
> [!tip] 找以顶点为一端端点的边
> 
> 从顶点 $v$ 的 `firstedge` 出发后，每经过一个边结点，都要判断顶点是该边的 `i` 端还是 `j` 端：若当前顶点是 `i`，下一步走 `iLink`；若顶点是 `j`，下一步走 `jLink`。
