...
**虚拟盘**通常指 **RAM 盘（RAM disk）**：为了提高访问外存的速率，操作系统从主存中划出一块区域，把它模拟成可格式化、可挂载、可按块读写的磁盘设备。应用程序仍通过文件和块设备接口访问它，但底层数据实际保存在 RAM 中。
...
3. **磁盘控制器**：主机和磁盘之间的接口，负责接收命令、控制磁盘读写并传输数据。主机不会直接操纵磁头和盘片，而是通过控制器发出读写命令。

从 [[IO-Device-And-Controllers#I/O 控制器（I/O 接口）|I/O 系统]]角度看，磁盘控制器就是一种 [[IO-Device-And-Controllers#I/O 控制器（I/O 接口）|I/O 控制器]]。CPU 或 DMA 控制器面对的是控制器中的寄存器和端口；寻道、选择磁头、等待扇区、校验和坏块替换等细节由磁盘控制器和磁盘驱动器配合完成。
