...
在 408 的存储器语境中，**虚拟盘**通常指 **RAM 盘（RAM disk）**：操作系统从主存中划出一块区域，把它模拟成可格式化、可挂载、可按块读写的磁盘设备。应用程序仍通过文件和块设备接口访问它，但底层数据实际保存在 RAM 中。

| 特点 | 原因 |
| --- | --- |
| 访问速度快 | 不需要机械寻道，也不必访问外部存储介质 |
| 占用主存容量 | 划给虚拟盘的空间不能同时供普通程序自由使用 |
| 通常不具备持久性 | 断电或重启后 RAM 中的数据会丢失，除非另行保存到外存 |

> [!warning] 虚拟盘与虚拟存储器方向相反
> 虚拟盘是**用主存模拟磁盘**，目的是得到高速的块设备；虚拟存储器是**借助外存扩展进程可见的内存空间**，目的是让程序获得大于当前物理内存的地址空间。二者不能因为都有“虚拟”二字而混为一谈。

在虚拟机语境中，“虚拟磁盘”还可能指提供给客户机的逻辑块设备，其后端是镜像文件、逻辑卷或真实分区，见 [[OS-VMM#虚拟机与 VMM|虚拟机的虚拟磁盘]]。这与 408 存储器题目中常说的 RAM 盘不是同一个概念。
...
从 I/O 系统角度看，磁盘控制器就是一种 [[IO-Device-And-Controllers#I/O 控制器|I/O 控制器]]。CPU 或 DMA 控制器面对的是控制器中的寄存器和端口；寻道、选择磁头、等待扇区、校验和坏块替换等细节由磁盘控制器和磁盘驱动器配合完成。
...
![1000](../assets/disk-address-order.svg)
...
[[Disk-Management#磁盘初始化|格式化]]后，磁盘需要保存扇区头、校验信息等管理内容，因此格式化容量通常小于非格式化容量。
...
关于具体计算式与减少延迟的方法详见[[Disk-Management]]

## 数据传输率

**数据传输率**是**数据真正开始传输后**，磁盘存储器单位时间内向主机传送数据的字节数。单位B/s。
