...
**系统调用**是应用程序请求操作系统内核服务的接口。凡是涉及共享资源、影响其他进程或需要特权操作的功能，都应通过系统调用进入内核。
...
1. 用户程序准备系统调用号和参数。
2. 用户程序执行 trap 指令，也叫陷入指令或访管指令。
3. CPU 产生自陷，硬件切换到内核态，并转入系统调用入口。
4. 系统调用入口根据系统调用号分派到具体内核处理程序。
5. 内核完成服务，设置返回值。
6. 通过返回指令回到用户态，用户程序继续执行。
