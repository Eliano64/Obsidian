...
内核维护的时间状态会影响所有进程，**可能允许普通程序读，但绝不允许普通程序可能改。**

| 操作                     | 权限判断   | 原因或实现方式                                    |
| ---------------------- | ------ | ------------------------------------------ |
| 修改系统日期、墙上时钟或硬件实时时钟 RTC | 特权操作   | 会改变全局时间，需要内核检查调用者权限                        |
| 修改内核时钟滴答数、时间片余量、定时器队列  | 特权操作   | 这些变量决定计时、调度和超时行为，位于内核空间                    |
| 读取当前时间或单调时钟            | 用户可以申请 | 可通过 `clock_gettime()` 等接口读取，内核状态仍不可由用户直接写入 |
| 读取进程已经使用的 CPU 时间       | 用户可以申请 | 内核统计后通过受控接口返回                              |
| 修改用户空间中的普通时间变量         | 非特权操作  | 只修改该进程自己的数据，不会改变系统时钟                       |

读取时间不一定每次都要通过[[System-Call|系统调用]][[#用户态到内核态|切换为内核态]]。操作系统可以把经过整理的时间数据以只读方式映射到用户空间，例如 Linux 通过 vDSO 加速部分时钟查询。某些处理器还允许用户态读取硬件计数器。
...
| 来源 | 例子 | 本质 |
| --- | --- | --- |
| 系统调用 | 用户程序执行 trap / 访管指令 | 程序主动请求内核服务，完整过程见 [[System-Call]] |
...
这里关心的是异常与中断如何改变 CPU 状态。其完整定义、分类和处理过程见 [[Exception-And-Interrupt-Definition]] 与 [[Exception-And-Interrupt-Handling]]。
