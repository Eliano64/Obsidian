...
如果本次读写字节数（即一个/若干相邻扇区字节数）为 $b$，每个磁道可存 $N$ 字节，则传输时间为：

$$
T_t=\frac{b}{rN}
$$

> [! danger] 如果磁道包含扇区数$M$与本次读写相邻扇区数$m$
> 此时 $$T_{t}= \frac{m}{rM}$$即磁盘转过所有扇区的时间。

因此总时间也可写成：

$$
T_a=s+mn+\frac{1}{2r}+\frac{b}{rN}\text{或}T_a=s+mn+\frac{1}{2r}+\frac{m}{rM}
$$
...
逻辑格式化由操作系统安装程序在分区之后立即执行。它在分区上建立文件系统，在设备提供的扇区之上选择自己的逻辑管理与分配单位，进而创建文件系统元数据：根目录、空闲空间管理结构（如空闲块链表或位图）、文件控制结构（如 inode）等。逻辑格式化完成后，操作系统才能按文件名创建、查找、读写和删除文件。日常使用中的"格式化 U 盘"就是这一层操作。

## 物理扇区、逻辑扇区与文件系统块/簇

这三个单位分属设备物理结构、设备接口和文件系统三层。文件系统块或簇的大小不是磁盘出厂时固定的，而是在逻辑格式化时由文件系统选择，并由一个或多个逻辑扇区组成。

| 层次 | 大小由谁决定 | 含义 |
| --- | --- | --- |
| 物理扇区 | 存储设备及其低级格式决定 | 介质实际读写和纠错的物理单位 |
| 逻辑扇区 | 设备接口向主机呈现 | 主机可寻址的基本块，常见为 512 B 或 4 KiB |
| 文件系统块或簇 | 创建文件系统时由文件系统格式和格式化参数选择 | 文件系统分配空间、建立索引和组织 I/O 的基本单位，通常包含一个或多个逻辑扇区 |
