...
| 机器周期 | 又称 CPU 周期，完成一个如取指、译码等基本机器操作的时间。一个指令周期通常由若干机器周期组成， |
| 时钟周期 | 又称节拍、T 周期，是 CPU 操作的基本时间单位                         |

指令的执行可用以下四个基本机器操作表示。

| 机器操作 | 对应机器周期 | 完成的工作              | 何时需要               |
| ---- | ------ | ------------------ | ------------------ |
| 取指   | 取指周期   | 从主存取出指令，并形成下一条指令地址 | 每条指令都需要            |
| 间址   | 间址周期   | 取有效地址              | 指令采用间接寻址时需要        |
| 执行   | 执行周期   | 完成指令规定的运算、访存或转移等操作 | 除空操作等特殊指令外通常需要     |
| 中断   | 中断周期   | 保存断点并转入中断服务程序      | 当前指令结束后检测到并响应中断时需要 |
