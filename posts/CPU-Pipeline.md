...
> [!info] 控制信号也要随流水线向后流动
> 控制信号也要被流水线寄存器保存。对一条指令来说，`ID` 阶段译码后会同时产生后续阶段需要的控制信号，这些控制信号也随着流水线向后流动。
...
> 
流水线寄存器保存的不只是数据值，还包括“这条指令到后面阶段时应该怎样控制数据通路”的信息。某一组控制信号在对应阶段用完后，就不需要再继续向后传。

# MIPS 五段阶段

| 阶段    | 名称                 | 主要工作                          |
| ----- | ------------------ | ----------------------------- |
| `IF`  | Instruction Fetch  | 根据 PC 从指令 Cache 取指令，然后 `PC+4` |
...
> [!note] R 型、I 型和 J 型
> MIPS 按**指令编码格式**将指令分为 R 型（寄存器型，用寄存器字段指出操作数和结果，具体运算由功能字段确定）、I 型（立即数型，立即数字段可表示常数、访存偏移量或分支偏移量）和 J 型（跳转型，用目标地址字段形成跳转地址）。
...
从时空图看，前 `k-1` 个周期主要用于把第一个任务逐步送入各级流水段；第 `k` 个周期开始，第一个任务完成。之后在没有停顿的理想情况下，每过一个机器周期就完成一个任务，直到第 `n` 个任务走完最后一级流水段。

> [!tip] 对于最后一段用时更少的流水线完成任务用时分析
> 前面的指令还是以耗时最长的段的耗时为 $\Delta t$, 算最后的指令时，就只用加上最后一段的更少的用时。因为它无需再对齐。
...
效率反映流水线中的硬件在整个执行过程中有多少得到实际使用。

执行 $n$ 个任务共需 $k+n-1$ 个周期，每个周期都有 $k$ 个流水段可工作，所以最多可进行 $k(k+n-1)$ 次流水段工作；而 $n$ 个任务各经过 $k$ 段，实际只进行了 $nk$ 次。两者之比就是流水线效率：
...
数据相关分为 WAW, WAR 和 RAW。前两者被称为假相关，本质原因是寄存器数量有限，可以通过寄存器重命名技术解决。 RAW 被称为真相关。

RAW 相关指后一条指令要**读**某个寄存器，而前一条尚未把这个寄存器的新值**写**出来。
...
| 方法         | 含义                         | 是否可以完全解决数据相关        |
| ---------- | -------------------------- | ------------------- |
| 硬件阻塞       | 检测到相关后插入气泡                 | 可以，通过等待保证相关指令取得正确数据 |
| 软件插入 `nop` | 编译器或程序员显式留出空周期             | 可以，前提是留出的空周期足够       |
| 数据旁路/转发    | 把后面阶段的结果直接送到前面需要它的 ALU 输入端 | 不能解决 load-use 型数据相关 |
| 编译优化       | 调整指令顺序，填补等待周期              | 不能保证，总有可能找不到可调度的独立指令 |
| 寄存器重命名     | 为存在名称相关的写操作分配不同的物理寄存器       | 不能解决 RAW；只能消除 WAR 和 WAW |
...
当分支方向和目标地址尚未确定时，流水线不知道下一条指令应该从哪里取，只能按默认的 `PC+4` 的顺序取。在分支/跳转指令 `EX` 阶段结束时才知道下一条指令的地址，这导致后续已进入流水线的错误路径的指令被冲刷掉。
