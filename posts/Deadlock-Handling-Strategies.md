...
在每次分配前判断：若同意这次请求，系统还能否保证所有进程最终完成。

## 安全状态与不安全状态

- **安全状态**：当前状态下，至少存在一种进程完成顺序，使每个进程都能依次取得尚需资源、执行结束并归还资源。这个顺序就是安全序列。
- **不安全状态**：对于当前状态，找不到安全序列。系统已经无法保证所有进程都能完成，但进程此刻未必已经互相等待。
- **死锁状态**：一组进程已经互相等待，若无外力干预便无法继续。死锁一定不安全，不安全却不一定已经死锁。

例如：

| 状态 | `P1`：Max / Allocation / Need | `P2`：Max / Allocation / Need | Available | 结论 |
| --- | --- | --- | --- | --- |
| 分配前 | `3 / 1 / 2` | `2 / 1 / 1` | `1` | `P2` 可先完成，再让 `P1` 完成，安全序列为 `P2 → P1` |
| 试探给 `P1` 一个实例 | `3 / 2 / 1` | `2 / 1 / 1` | `0` | 无进程能取得剩余需求，进入不安全状态 |
...
资源数为 `Available`，初始值即全部资源数减去已分配的资源数。**对于每一个进程，只关注 `Allocation` 和 `Need`。**

1. 在尚未完成的进程中寻找 `Need <= Work` 的进程。
2. 把它加入安全序列，回收它原先占有的资源：`Available += Allocation`。
3. 继续寻找；若仍有未完成进程却找不到满足条件者，则当前状态不安全。

对前面的例子，可以得到：

| 当前资源       | 选择   | `Need`    | 完成后 `Work + Allocation` |
| ---------- | ---- | --------- | ----------------------- |
| `(3,3,2)`  | `P1` | `(1,2,2)` | `(5,3,2)`               |
| `(5,3,2)`  | `P3` | `(0,1,1)` | `(7,4,3)`               |
| `(7,4,3)`  | `P0` | `(7,4,3)` | `(7,5,3)`               |
| `(7,5,3)`  | `P2` | `(6,0,0)` | `(10,5,5)`              |
| `(10,5,5)` | `P4` | `(4,3,1)` | `(10,5,7)`              |

因此一个安全序列是：

$$
P1 \rightarrow P3 \rightarrow P0 \rightarrow P2 \rightarrow P4
$$

> [!danger] 进程完成后给 `Available` 加回 `Allocation` **而不是 `Need` **。
...
判断一种措施属于哪类策略，要看它发生在什么时候、依据什么作出决定。

| 判断线索                                                 | 所属策略 | 典型做法               |
| ---------------------------------------------------- | ---- | ------------------ |
| 预先制定固定分配规则，使四个必要条件之一无法成立                             | 预防死锁 | 一次申请全部资源、按编号递增申请资源 |
| 每次分配前根据当前 `Available`、`Allocation`、`Need` 判断分配后是否仍安全 | 避免死锁 | 银行家算法试探分配，不安全就撤销   |
| 不保证每次分配后安全，运行过程中再检查是否已经形成死锁                          | 检测死锁 | 化简资源分配图，找出无法消去的进程  |
| 已经检测到死锁后，破坏当前等待关系                                    | 解除死锁 | 剥夺资源、撤销进程、进程回退     |

> [!warning] 相同动作可能属于不同策略
> - **拒绝一次资源请求**：若因为请求违反固定的资源**申请顺序**，属于预防；若因为试探后发现系统将进入不安全状态，属于避免。
> - **剥夺资源**：若系统预先规定必要时可剥夺，从而破坏“不剥夺条件”，属于预防；若在检测到死锁后才剥夺，属于解除。
