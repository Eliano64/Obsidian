...
C 程序经过[[C-And-Cpp-Compilation#从源码到可执行文件的四个阶段|编译、汇编、链接]]后得到目标程序文件，操作系统再为它建立进程地址空间中的用户部分，并准备第一次执行所需的现场。
...
2. **装入**：根据可执行文件建立进程最初的用户地址映射、用户栈和运行现场。
...
**装入**是操作系统根据可执行文件创建进程初始用户地址空间和执行现场的过程。现代系统通常按文件段建立虚拟内存映射，并在发生缺页时才把具体页面读入主存，并非启动时把整个可执行文件一次性复制到物理内存。

装入程序通常完成：

1. 读取可执行文件头，取得入口地址、各可装入段的位置、大小、文件偏移和权限。
2. 创建进程的用户虚拟地址空间，把代码、只读数据和数据段映射到相应虚拟地址。
