...
| 类型   | 传什么                             | 方向                | 位数或根数的含义           |
| ---- | ------------------------------- | ----------------- | ------------------ |
| 数据总线 | 各功能部件之间的**数据信息**，包括指令和操作数、中断类型号 | 双向                | 位数与机器字长、存储字长有关     |
| 地址总线 | 主存单元或 I/O 端口的地址                 | 通常单向，由 CPU 或主设备发出 | 位数与主存地址空间大小、设备数量有关 |
| 控制总线 | 控制命令和反馈信号                       | 双向                | 一根控制线通常传一种控制或状态信号  |
...
CPU、主存、通道和 DMA 控制器连接在主存总线上。通道向下连接 I/O 总线，管理普通 I/O 设备；DMA 控制器向下连接 DMA 总线，使高速 I/O 设备可以直接与主存交换数据。
...
CPU 先接入 CPU 总线；桥接器一侧连接 CPU 总线，另一侧连接主存总线，并向下连接局部总线；第二级桥接器再把局部总线连接到更低速的 I/O 总线。
