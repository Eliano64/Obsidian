...
**异构网络**指底层实现不同的网络。它们可能使用不同的链路层协议、帧格式、链路层地址、最大帧长、差错处理方式和接入机制。

网络层用统一的 IP 地址和统一的转发规则，把这些异构网络的差异屏蔽起来，对上提供一个可互联的 IP 网。
...
虚电路服务是面向连接的网络层服务。发送数据前，源主机先向网络发出连接建立请求，网络根据目的地址选择一条路径。路径上的每个节点为该连接建立转发表项，记录“从哪个接口、以哪个虚电路号进入，应从哪个接口、改用哪个虚电路号发出”。目的端接受后，确认报文沿途返回，虚电路建立完成。

建立虚电路必须进行一次路由选择，但不一定预留带宽。是否预留带宽、缓冲区等资源，取决于网络是否提供相应的服务质量保证；虚电路本身只表示网络中已经建立了逻辑连接和沿途转发状态。

[html-card height=650](../assets/virtual-circuit-service-slides.html)

虚电路不是物理电路。连接建立阶段需要完整目的地址；连接建立后，后续分组只携带较短的虚电路号，沿途节点按已经建立的转发表项转发。
