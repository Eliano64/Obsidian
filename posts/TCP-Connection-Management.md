...
第三个报文可以携带数据。若它只是一个纯 ACK，就**不消耗序号**；`seq=x+1` 只是表明客户端下一个数据字节应从 `x+1` 开始。若该报文同时携带 $n$ 字节数据，消耗序号的是这 $n$ 字节，发送后的下一个序号为 `x+1+n`。
...
最后一个报文若只是纯 ACK，同样**不消耗序号**。表中的 `seq=u+1` 是客户端发送 `FIN` 后的下一个可用序号；若最后 ACK 丢失，客户端还可以用相同序号再次确认服务器重传的 `FIN`。
