...
> [!warning] ROM 与 RAM 不是对立的
> ROM 强调“只读或写入受限”。ROM 是只读存储器，**但其读取方式属于随机存取。**
# 层次化结构
...
多级存储结构通过相邻层次间的数据调度发挥作用：Cache 与主存用块交换缓解速度差，主存与辅存用页交换扩展可用容量。


![622](../assets/memory-system-three-level-flow.svg)
...
> [!note] 透明性
> Cache 和 虚拟内存中的页面对于用户透明：Cache 的调入由硬件自动完成；虚拟内存中的页面调入调出由操作系统和硬件配合完成。

# 主存储器的基本组成

![668](../assets/main-memory-cpu-interface.svg)
...
2. 地址转换机构查询 TLB。TLB 命中则直接得到物理页框号。
	1. TLB 未命中时，访问页表，查出虚拟页对应的物理页框。
	2. 如果页表项无效，则目标页不在主存，则发生缺页，由操作系统把页面从辅存调入主存。
3. 得到物理地址后，硬件用物理地址访问 Cache。
4. Cache 命中则直接返回数据。
	1. Cache 未命中则访问主存，把对应主存块调入 Cache，**同时**返回数据。

> [!warning] TLB 和 Cache 的区别
> TLB 缓存的是页表项，负责虚拟地址到物理地址的转换。目标是减少地址转换开销；
> Cache 缓存的是主存数据块或指令块，负责存储数据。目标是减少主存访问开销。
