...
> [!warning] “平衡”不是指各归并段长度相等
> 它是指同一趟中每次尽量取满 $k$ 个归并段，只有最后剩余不足 $k$ 个时才允许少取。例如 $8$ 个归并段做 $4$ 路平衡归并，应分成 $4+4$ 两组，一趟得到 $2$ 个新归并段；若分成 $4+2+2$ 三组，虽然每组都没有超过 $4$ 路，却没有充分使用归并路数，因此不属于 $4$ 路平衡归并。
