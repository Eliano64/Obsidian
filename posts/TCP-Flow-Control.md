...
| --- | -------------------------------- | --------------------------------------------------------- | ----------------------------------------------------------- |
| ①   | 连续发送 `1-100`、`101-200`、`201-300` | （无） | `400→300→200→100`（已发未确认逐渐填满窗口）                              |
| ②   | （无） | ACK=201, rwnd=300（第一次流量控制：B 的应用进程读取较慢，缓存可用空间从 400 缩到 300） | `swnd → 300`，窗口覆盖 `201-500`。其中 `201-300` 已发未确认，`301-500` 可发 |
| ③   | 发送 `301-400`、`401-500`           | （无） | 可用窗口归零，A 暂停发送                                               |
