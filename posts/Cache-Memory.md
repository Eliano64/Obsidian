...
全相联映射中，一个主存块可以放入任意 Cache 行。地址中没有行号或组号，故没有没有 `Index` 位。于是主存地址被表示为：
...
5. 若 `Valid=1` 且 `Tag` 相等，则命中；再用 `Block Offset` 取出块内目标数据。
	1. 若没有候选行满足条件，则未命中，需要从主存找到目标块，**直接**送CPU，**同时**写入Cache。
...
| 命中率 $H$ | 访问命中的比例                   |
| 缺失率 $M$ | $M = 1 - H$               |
| 缺失损失    | 未命中时相较于命中时访问所需的**额外的时间**。 |

* 默认先访问 Cache，未命中后再访问主存：
