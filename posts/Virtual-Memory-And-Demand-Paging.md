...
虚拟存储的核心思想是：基于局部性原理，只把当前需要的页面装入主存；暂时不用的页面留在外存；访问到不在主存的页面时，再由操作系统把它调入。

虚拟存储技术是补充相对地址空间的技术。在用户看来，系统好像提供了一片比实际主存更大的内存；实际物理主存没有变，只是操作系统和硬件共同把主存与外存组织成了一个逻辑上的大地址空间。
...
> [!note] 虚拟存储器的最大容量
> 虚拟存储器的最大容量由虚拟地址长度确定。
> 但实际能实现的最大的虚拟内存容量还受主存与外存容量限制，即$\min(2^n，\text{可用内存容量+可用外存容量})$
>
> 例如 32 位按字节编址的系统，CPU 寻址范围最大是 $2^{32}B=4GB$。若主存为 512MB，外存可供虚拟存储使用 2GB，则实际实现的容量不是 4GB，而是 2.5GB。
...
因为页表通常采用数组实现，可用数组下标表示虚拟页号。**所以虚拟页号是隐式的，不需要专门设置虚拟页号字段。**

状态位是请求分页的关键：状态位为 1，说明页面在主存；状态位为 0，说明页面不在主存，访问该页会触发缺页故障。

> [! Info] 请求分页系统语境下的进程建立
> 在请求分页系统中，进程被装入并具备运行条件，实质上主要是建立 PCB、虚拟地址空间和页表，同时部分页面可能装入内存。对于尚未调入内存的页面，页表会记录虚页号对应页面在外存中的位置；访问时再通过触发[[#缺页故障]]将其调入。
...
2. MMU 用虚拟页号查 TLB。
	...
	2. 若 TLB 未命中，继续查页表。
		...
		2. 若页表项状态位为 0，说明页面不在主存：产生**缺页故障**，转入 [[#缺页故障|OS 缺页处理]]；调页完成后重新执行原访存指令。
...
> [!note] **TLB 命中意味着对应页面一定在主存。**
> 若某页面被换出，OS 必须使对应 TLB 项失效，否则硬件可能拿着过期页框号访问错误位置。

## 缺页故障

**缺页故障**是指 CPU 因执行访存指令而进行地址转换时， MMU 查页表发现目标页面尚未调入主存，于是 CPU 产生的可恢复异常。处理程序把缺失页面调入主存并更新页表后，通常返回原指令重新执行。
...
1. CPU 执行访存指令，产生虚拟地址。
2. MMU 查 TLB 或请求页表。若页表项状态位为 0，硬件产生缺页故障，转入 OS 缺页处理程序。
3. OS 保存现场，将缺页进程阻塞。
4. OS 判断访问是否合法；若非法，终止进程；若合法，继续调页。
5. 若有空闲页框，直接分配；若没有空闲页框，按页面置换算法选择换出页。
	1. 若被换出页修改位为 1，写回外存；修改位为 0，则不必写回。
6. 根据页表项中的外存地址，把缺失页面从外存读入页框。
7. 修改页表项：填入页框号，状态位置为 1，更新访问字段、修改位等。
8. 必要时把页表项复制到 TLB。
9. 唤醒进程，重新执行引发缺页的原指令。
...
进程执行指令时使用虚拟地址；页面换入、换出的磁盘传输若采用 [[IO-Control-Methods#DMA 方式|DMA]]，设备使用的是内核为它准备的物理目标。

例如，进程 P1 访问的页面不在主存，内核决定把它调入 5 号页框：

1. 内核把 5 号页框标记为正在进行 I/O，在传输完成前禁止回收和重新分配。
2. 内核把 5 号页框对应的物理地址交给 DMA，启动磁盘读取。
3. P1 因等待页面而阻塞，调度器可以让其他进程运行。切换当前进程和页表不会改变 5 号物理页框，DMA 因而可以继续传输。
4. I/O 完成中断到来后，内核更新 P1 的页表，解除页框固定并唤醒 P1。

这里不是 DMA 自己“锁住”页框，而是内核先固定页框，再配置 DMA。其他进程不能在传输期间获得这个页框，是因为内存分配器不会分配正在进行 I/O 的页框。

因此，页面换入不一定需要另设页缓冲区：5 号页框本身就是数据最终要进入的位置。换出时也是同理，干净页可以直接丢弃；脏页可以把原页框固定到写回完成，再将它重新分配。

> [!important] 页框固定不等于页面缓冲
> 页框固定保证正在进行的 I/O 使用稳定的物理内存，是正确性要求；页面缓冲算法暂时保留刚被换出的页面，以便延迟写回或快速恢复，是性能优化。
...
> [!note]
> 页面缓冲算法不改变“缺页时要选页换出”的事实。它优化的是换出后的页框回收和再次访问路径：被换出的页不立刻进入外存，而是在缓冲队列中保留一段时间。
