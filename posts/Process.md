...
实际内核常把复杂信息放在其他内核结构中， PCB 仅保存指向该结构指针。例如，地址空间的大小由页表记录（页表大小）不存在 PCB 中，但 PCB 保存指向页表的指针。
...
父子关系只说明“谁创建了谁”。子进程不是父进程内部的一段代码：它有自己的 PID、地址空间和进程控制信息，由调度器独立调度。
