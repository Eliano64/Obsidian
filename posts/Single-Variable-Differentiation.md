...
在**一点**存在导数则在**一点**连续，但反之不成立。连续只是必要条件，但不连续一定不可导。

在一点存在导数**不能说明在邻域连续**，反例：[[#衰减狄利克雷函数]]。但如果在$x_{0}$一个**邻域**存在导数，**就可以说明在该邻域连续**。

> [!danger] $f'(x_0)$ 与 $f'(x)$ 不要混淆 
>
> $f'(x_0)$ 是一个**数**；$f'(x)$ 是**函数**。对 $x\ne x_0$ 求出的公式在 $x_0$ 无定义，不代表 $f'(x_0)$ 不存在。
> 
> 同样地，$\displaystyle\lim_{ x \to x_{0}^{\cdot} }f'(x_{0})\neq f'_{\cdot}(x), \cdot \text{为} + \text{ 或 } -$，见[[#^e7d1e0|衰减振荡函数]]
> 
> 所以求分类函数的分界点的导数必须单独用定义计算。即计算$f'_{-}(x_{0})$和$f'_{+}(x_{0})$而不能直接使用$f'(x)$的$x_{0}$左右两侧极限。

# 导函数的间断

**这里讨论的是 $f$ 在 $[a,b]$ 上连续、在 $(a,b)$ 上可导的情况。** ^001c24

导函数具有 [[Mean-Value-Theorem#Darboux 定理|Darboux 性质]]，即不能跳过中间值。因此**导函数只能连续或存在振荡间断点**。

**若导函数在一点附近有有限极限，该极限必须等于该点导数。反之不成立，见[[#衰减振荡函数]]** ^84c220

有第一类间断点和无穷间断点的函数不可能是某个满足[[#^001c24|前提条件]]的函数的导函数，也就没有满足这个条件的原函数。
...
由区域可导 $\Rightarrow$ 区域连续，又由极限存在，得 $f(0)=0$。再由导数定义，
