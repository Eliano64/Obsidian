...
指令是计算机执行某种操作的命令，是计算机运行的最小功能单位。一台机器能够识别并执行的全部机器指令构成它的**指令系统**，也称为指令集体系结构（Instruction Set Architecture，ISA）。

指令系统由计算机体系结构决定。它处在软件和硬件之间：高级语言程序最终必须被翻译成机器指令；硬件则按指令中的字段完成取数、运算、访存、转移和控制等动作。不同体系结构的机器指令格式和指令含义不同，因此一台机器通常只能直接执行自己指令系统中的指令。

# ISA 规定什么

**ISA 规定程序员可见的指令、数据、寄存器、地址空间和系统控制机制**。底层硬件的具体实现与参数 ISA 不规定。

具体包括：

- 数据类型及数据格式、数据存储规则；
- 指令的功能、格式和寻址方式；
- 程序可访问寄存器的数量、位数、编号和用途，以及控制寄存器的定义；
- 可访问地址空间的大小、存储器编址方式和 I/O 空间编址方式；
- 中断结构、机器工作状态及其切换、I/O 数据传送方式和存储保护方式。
...
> - **机器字长**：定点运算数据通路一次作为一个整体能够容纳、传送或运算的二进制位数，常与通用寄存器位数、ALU 位数相关。
...
| 输入输出指令 | 在 CPU 和1外设接口之间传送数据或状态 | 端口输入、端口输出 |
