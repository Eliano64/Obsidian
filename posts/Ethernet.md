...
**冲突域**是共享同一传输介质、同时发送时可能发生信号碰撞的一组接口或站点。冲突域主要用于描述共享式、半双工以太网；全双工点到点链路没有共享介质争用，因此不会发生以太网碰撞。
...
因此，共享式与交换式以太网的核心区别可以先从冲突域看出：集线器把所有端口接入同一个冲突域，交换机则把不同端口分成独立链路。
...
> [!note]
> 交换机隔离**冲突域**，但默认不隔离**广播域**。广播帧仍会被交换机泛洪到同一 VLAN 内的其他端口。
