...
三种深度优先遍历只改变“访问根”的时机，对于左右子节点都是先左后右：
...
递归遍历依赖函数调用栈。非递归算法需要显式使用[[Stack-Definition-And-Operations|栈]]模拟函数调用过程。

更一般的递归转循环思路见[[Recursion-To-Loop-With-Stack]]：栈中保存的不是单纯的结点指针，而是一个“栈帧”，其中至少包含当前结点和执行位置 `ip`。
