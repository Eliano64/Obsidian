...
链接器的输入是目标文件和库，输出通常是可执行文件或共享库。目标文件已经包含机器指令，但还不是一个完整程序，此时保存：

| 内容      | 作用                     |
| ------- | ---------------------- |
| 代码节、数据节 | 保存本模块生成的机器码和数据，即`.o`文件 |
| 符号表     | 记录本模块定义或引用的函数、全局变量     |
| 重定位项    | 标出哪些指令或数据中的地址仍需修正      |

例如 `main.o` 中有一条调用 `sum` 的指令，而 `sum` 定义在 `math.o`：
...
汇编器生成 `main.o` 时还不知道 `sum` 最终位于哪里，只能留下对符号 `sum` 的未解析引用，并生成一条重定位记录。链接器随后完成四项工作：

1. **合并多个 `.o` 文件**：把各目标文件的代码节、只读数据节、数据节等组织到输出文件的相应区域。
2. **解析函数调用、全局变量等符号地址**：把 `main.o` 对 `sum` 的引用与 `math.o` 中 `sum` 的定义对应起来；缺少定义或出现冲突定义时报告链接错误。
3. **分配地址**：确定各输出区域以及函数、全局变量在输出地址空间中的位置。
4. **执行重定位**：根据重定位项，把调用指令或数据中的地址修正为最终值；若留给动态链接器处理，则在可执行文件中保留动态符号和动态重定位信息。
