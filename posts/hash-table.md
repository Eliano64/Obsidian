...
失败查找必须从每个**可能的初始地址**出发，探测到第一个空单元为止，空单元也计入一次。初始地址 `0～10` 的失败长度依次为 `9、8、7、6、5、4、3、2、1、1、1`，因此：

$$
ASL_{\text{失败}}=\frac{9+8+7+6+5+4+3+2+1+1+1}{11}=\frac{47}{11}\approx4.27
$$

分母是 `11`，因为 $H(key)$ 只有 11 种结果。地址 `11、12` 可以在冲突探测时存放元素，却不可能成为初始散列地址。
