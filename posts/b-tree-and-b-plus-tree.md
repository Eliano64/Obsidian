...
7. 所有叶结点都在同一层。这里的叶结点是外部失败结点，实际存储中常为空指针，画图时通常省略。

> [!important]
> 最底层含关键字的结点为终端结点。
> 真正的叶结点是它们下面的失败结点，通常不画。
...
| 应用        | 以单个关键字随机查找为主的外存索引             | 数据库、文件系统索引，尤其适合顺序和范围查找    |
