...
广域网 WAN（Wide Area Network）覆盖范围大，常用于连接相距较远的局域网、园区网、数据中心或运营商网络。
...
| 典型拓扑    | 现代局域网多采用以交换机为中心的星形拓扑；早期也有总线形、环形 | 由路由器和交换节点通过多条链路互连，整体可形成网状、树形或星形结构  |
| 传输方式    | 可使用共享广播信道，也可在交换式局域网中通过点对点链路传输帧  | 分组通常在交换节点之间逐跳转发；专线也可直接连接两个节点       |
| 典型链路形态  | 广播链路或交换式点对点链路                   | 点对点链路、交换网络、运营商承载网络                 |
| 涉及层次    | 物理层和数据链路层                       | 单条接入或中继链路涉及物理层、数据链路层，跨节点通信还依靠网络层路由 |
| 数据链路层重点 | MAC、局域网帧、交换机、VLAN               | 点对点封装、链路建立、链路配置                    |
| 典型协议    | Ethernet、802.11                 | PPP、HDLC 等                         |

广域网描述的是一张跨地域的网络，PPP 只负责其中的**一段点对点链路**。例如两台路由器之间使用专线连接时，PPP 规定这条链路怎样建立、配置、成帧和检错；分组离开这条链路后，后续各跳可以使用其他数据链路层协议。因此，下面从广域网的链路形态缩小到其中常见的点对点链路，再讨论 PPP。
...
PPP 是**面向连接**的。这里的连接指数据链路层连接。物理层连接可用之后，PPP 还要通过 LCP、可选鉴别、NCP 等步骤，把这条链路配置成能承载网络层数据的数据链路。
...
面向字节的**异步链路**使用 `0x7D` 作为转义字节。发送方遇到需要转义的字节 $x$ 时，发送：

$$
\texttt{0x7D},\quad x\mathbin{\mathrm{XOR}}\texttt{0x20}
$$
...
| `0x7E` | `0x7D 0x5E` |
| `0x7D` | `0x7D 0x5D` |
| `0x11` | `0x7D 0x31`，前提是协商要求转义该控制字符 |

哪些 `0x00`～`0x1F` 控制字符需要转义，由异步控制字符映射 ACCM 指定，并非任何链路都固定转义全部控制字符。接收方读到 `0x7D` 后，将紧随其后的字节与 `0x20` 异或，便可恢复原字节。例如 `0x31 XOR 0x20 = 0x11`。

## 零比特填充

面向比特的**同步链路**按比特流处理数据。PPP 的 Flag 是：
