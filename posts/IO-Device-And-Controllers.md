...
从设备一侧看，控制器通过**外部接口**连接具体设备。外部设备不一定按主机总线的格式工作，数据可能是串行传输，也可能有设备自己的控制时序。因此 I/O 控制器常要完成串并转换、格式转换和速度匹配。

CPU 访问控制器时，地址和命令先由内部接口送入，I/O 逻辑据此选择寄存器并控制外部接口；数据和状态则通过相应寄存器在主机与设备之间传递。

I/O 控制器通常包含以下部分：

| 组成部分   | 作用                                                   |
| ------ | ---------------------------------------------------- |
| 内部接口   | 连接系统总线，接收 CPU 送来的地址、命令和输出数据，并向主机返回输入数据和设备状态          |
| I/O 端口 | 数据寄存器暂存输入或输出数据；状态寄存器保存忙、就绪和出错等状态；控制寄存器保存 CPU 写入的控制命令 |
| I/O 逻辑 | 实现对设备的控制功能：根据地址选中寄存器或设备，解释控制命令，安排传送时序，并在需要时发出中断请求    |
| 外部接口   | 把控制器产生的控制信号送给设备，并接收设备返回的数据和状态信号                      |
...
**I/O 指令**是 CPU 指令系统的一部分，用来让 CPU 对 I/O 控制器或通道发出命令，属于特权指令。它通常需要表达两类信息：

- 对哪个控制器、设备或端口操作；
- 做什么操作，例如读、写、启动、查询状态。

在有通道的系统中，CPU 不再逐项管理大量设备操作，而是执行 I/O 指令启动通道。通道再执行**通道程序**。

操作系统中的设备驱动程序根据用户的 I/O 请求，组织一系列通道指令，形成通道程序并将其放入主存。通道程序中的**通道指令**描述要访问的设备、传输的数据量、数据在主存中的位置以及传输方向。
