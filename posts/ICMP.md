...
ICMP 用来传递网络层控制信息。它封装在 IP 数据报中，是网络层协议；它服务于 IP，帮助主机和路由器报告差错、进行连通性测试和路径探测。
...
**参数问题**表示**首部已经足够可信、能够继续解释它**，但发现其中某个具体字段的取值/语义有问题时，才可能发送参数问题。所以，当首部校验和出问题时，首部完全不可信，于是路由器会直接丢弃。
...
PING 使用 ICMP 询问报文，回送请求和回送回答检查目的主机是否可达。
