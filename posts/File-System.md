...
其中，目录、FCB、inode 和分配结构属于文件系统自己的管理对象；真正启动磁盘读写时，会继续下沉到 [[Disk-Management|磁盘调度与磁盘管理]]、[[IO-Control-Methods|I/O 控制方式]] 和设备驱动程序。也就是说，文件系统决定“读哪个文件块”，I/O 子系统负责“怎样让设备把这个块传到主存”。

## 从路径到文件数据

文件系统处理一次文件访问时，需要先后解决两个定位问题，并为进程保存本次打开的状态：

| 工作          | 输入     | 得到的结果                 |                          
| ----------- | ------ | --------------------- | 
| 目录查询 | 文件路径名                 | 目标文件的 FCB；或目标目录项及其 inode |
| 地址转换        | 文件逻辑地址 | 文件所在的物理盘块及块内位置        |                        
| 打开文件管理      | 本次打开操作 | 文件描述符、读写位置、访问方式等运行时信息 |                        

执行 `open(path, ...)` 时，操作系统先逐级查询目录，取得目标文件的控制信息并建立打开文件表项。后续的 `read(fd, ...)` 和 `write(fd, ...)` 可以通过文件描述符直接找到打开文件表项，不必重新按路径查询目录。
...
| 延迟写回            | 先修改缓存中的页，稍后合并写入已分配好的盘块 | 减少零散写 I/O；断电前未落盘的数据可能丢失                 |
| 延迟分配            | 数据先留在内存，写回磁盘时才分配盘块     | 可以按整批数据的大小分配连续盘块；空间不足可能到写回时才发现          |
| 连续或相邻分配         | 尽量给同一文件分配一段连续的盘块/簇     | 顺序访问时减少寻道并提高吞吐量；需要合适的连续空闲区，长期使用后可能受碎片影响 |
