...
| 延迟分配 | 写入刚进入页缓存时暂不决定具体盘块；等累计了一批脏页、准备写回时，再根据实际长度一次选择盘块 | 更容易为整批数据找到连续空间；延迟期间崩溃的数据尚未落盘 |
| 连续或相邻分配 | 尽量给同一文件分配一段连续的盘块、簇或 extent；空间不足时也优先选择彼此接近的区域 | 顺序访问时减少寻道并提高吞吐量；需要合适的连续空闲区，长期使用后可能受碎片影响 |

“为文件分配连续的簇”属于连续或相邻分配。延迟分配解决的是**何时选盘块**，连续或相邻分配解决的是**选哪些盘块**；两者可以配合，但不是同一种方法。
