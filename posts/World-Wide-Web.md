...
图中先取得基本 HTML，再根据其中的 URL 请求图片，因此两个对象分别经历两条 TCP 连接。每条连接都包含建立、HTTP 请求与响应、释放三个阶段；释放画成四报文挥手，是因为 TCP 的两个传输方向要分别关闭。
...
这里的 $2\text{ RTT}$ 是**取得一个对象的经典时延近似**，不把对象传完后的 TCP 释放时间算进去。挥手仍然会发生，只是用户已经收到该对象，而且连接释放可以与浏览器解析对象、准备后续请求等工作重叠。
