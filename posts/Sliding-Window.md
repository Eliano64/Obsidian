...
发送窗口表示发送方在收到下一次确认之前等待确认的数据和可以发送的数据，长度 $W_{T}$ 可以设定，左边界是最早未被确认的数据或未发送的数据。

窗口内的数据分为两类：

- 已发送但未确认：等待确认，保留在发送缓存中，超时后可重传。
- 尚未发送：在收到确认前可以发送。

当左边界的数据收到 ACK 后，发送方把它移出窗口，左边界右移，于是窗口右边进入新的尚未发送的数据。

> [!note] 在收到下一次确认前可以发送的数据量
> 即在窗口中的尚未发送的数据量。于是：
> $\text{在收到下一次确认之前可以发送的数据量}=W_{T}-\text{已发送但未确认的数据量}$
# 接收窗口
...
信道利用率指出某信道有百分之几的时间是被利用的（有数据通过）。是**本次用于***发送***数据的时间与本次发数据开始到下次发数据开始的时间间隔的比值**。
