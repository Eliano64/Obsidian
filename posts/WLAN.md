...
共享式以太网的 CSMA/CD 见 [[Ethernet#共享式以太网]]。
...
![900](../assets/wlan-infrastructure-vs-ad-hoc.svg)
...
![622](../assets/wlan-csma-cd-vs-ca.svg)
...
- 发送前监听信道。
- 信道空闲后先等待帧间间隔。
- 信道忙后再争用、发送失败需要重传，或成功发送后还要连续发送下一帧时，进行随机退避。只有站点发送首帧且信道已经持续空闲足够长时间时，才可以在等待 DIFS 后直接发送。
- 数据帧发送后**等待 ACK**。
...
1. 站点有帧要发，先监听信道。
	1. 若信道忙，站点不发送，等待信道从忙变为空闲。
2. 信道空闲后，站点先连续等待一个 DIFS。若 DIFS 期间信道又变忙，则回到等待信道空闲。
3. DIFS 结束后，站点从当前争用窗口 CW 中随机选一个整数 $r$，表示还要等待 $r$ 个时隙。
4. 每经过一个空闲时隙，退避计时器减 1。
	1. 如果倒计时过程中信道变忙，计时器立即冻结，剩余值不清零。
5. 等信道再次空闲，并重新等待 DIFS 后，计时器从冻结时的剩余值继续倒计时。
6. 当计时器减到 0，站点获得发送机会，开始发送数据帧。
...
物理载波监听回答“我现在是否听到无线信号”。虽然由于[[#隐藏站问题]]，无线局域网不能直接沿用这样的监听思想，但它可以实现“我虽然听不到某个站点，但我知道信道已经被预约”。

802.11 引入**虚拟载波监听**。其他站点听到 RTS、CTS 或 DATA 后，计算这次交换距离结束还剩多久，并让 NAV（Network Allocation Vector，网络分配向量）从这个时间开始倒计时。具体计算见[[#NAV 计算]]。
...
>[!important] 虚拟载波监听可以解决隐藏站问题
>B 听不到 A 的 RTS，但可能听得到 AP 的 CTS。B 听完 CTS 后，算出后续的 SIFS、DATA、SIFS 和 ACK 还要持续多久，并把 NAV 设为这段时间。

## 基本发送过程

展示一个普通数据帧的 CSMA/CA 发送过程：源站等待 DIFS 后发送数据，目的站等待 SIFS 后返回 ACK；其他站点让 NAV 覆盖到 ACK 结束，在此之前推迟发送。
...
2. 源站发送 RTS（Request To Send），请求目的站配合完成后续发送。
3. 目的站正确收到 RTS 且信道可用，等待 SIFS 后发送 CTS（Clear To Send）。
4. 源站收到 CTS 后，等待 SIFS，再发送数据帧。
5. 目的站正确收到数据帧后，等待 SIFS，返回 ACK。
6. 其他站点听到 RTS、CTS 或 DATA 后，把 NAV 设置为从当前帧结束到本次交换结束的剩余时间。

## NAV 计算

隐藏站 C 听完 CTS 后，后续还要经过 SIFS、DATA、SIFS 和 ACK。因此：

$$
\text{NAV}=2T_{\text{SIFS}}+T_{\text{DATA}}+T_{\text{ACK}}
$$

若 SIFS 为 $16\ \mu s$，DATA 和 ACK 的发送时间分别为 $300\ \mu s$ 和 $44\ \mu s$，则：

$$
\text{NAV}=2\times16+300+44=376\ \mu s
$$

C 在这 $376\ \mu s$ 内不发送，NAV 归零后才能重新争用信道。
...
CSMA/CA 的 ACK、RTS、CTS 属于控制帧；扫描和关联依赖管理帧。
...
最常见的是中间两种。对于中间两种情况，可以这样记忆：只考虑目的站，源站，AP三个mac地址。这个帧直接到达的即为地址1；帧从哪里直接发出来的即为地址2；剩下的就是地址3。
