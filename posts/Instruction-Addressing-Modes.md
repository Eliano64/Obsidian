...
寻址的最终目的是找到下一条指令或本条指令要使用的操作数，前者称为**指令寻址**，后者称为**数据寻址**。而为了找到对应的指令或操作数，需要先确定存储位置。
...
- 优点：执行阶段不访问主存，速度快；寄存器编号位数通常比主存地址短，**能够缩短指令的某个地址字段位数。**
- 缺点：寄存器数量有限。
...
基址寻址、变址寻址和相对寻址都属于偏移寻址。除了指令中明确给出的地址外，它们还需要一个寄存器给出一个基准值，共同的形式是：
...
基址寻址的典型作用是支持[[Memory-Management-Overview#装入|程序重定位]]。程序装入主存的位置可能变化，但指令中的偏移量可以不变，只要把 `BR` 设置为新的起始地址即可。
...
>[!warning]
计算$EA$时, 需要先将$A$符号扩展，再类型提升为无符号数才能与$(PC)$相加。

相对寻址广泛用于转移指令和浮动程序。代码整体在程序内部移动时，只要转移目标与当前指令之间的相对距离不变，转移指令的地址码就不需要修改。

>[!info]- 浮动程序
> 程序装入内存后，其占用的物理地址仍可以改变，而程序能够继续正确运行。
> 这样的程序需要代码**位置无关**，即一段机器代码无论被装入内存的什么位置，都无需对代码直接给出的地址进行*任何额外的转化操作*就能正确运行。
> 显然它们对内部指令和数据的访问使用相对地址或间接地址。
> 于是天然支持位置无关代码的相对寻址广泛用于浮动程序。
