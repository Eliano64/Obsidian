...
所以对于有符号数加减时，若要看 `CF` 的情况，需要先**以无符号数的解读方式解读两操作数**，然后[[#CF：无符号进位或借位|按规则]]判断。

# 条件转移与标志位

比较指令先设置标志位，条件转移指令再读取这些标志决定是否修改程序计数器。以 x86 为例：

```asm
cmp eax, ebx        ; 计算 eax - ebx，只更新标志位，不保存差
```

`cmp` 不知道操作数是有符号数还是无符号数。后续使用哪一组跳转指令，才决定如何解释同一组 bit 和标志。

![760](../assets/condition-branch-flags.svg)

## 相等关系

相等比较只关心结果是否为 0，与有符号、无符号无关：
...
有符号比较不能只看 `SF`。**若减法发生补码溢出，截断结果的符号会反转，因此还要用 `OF` 修正**：
