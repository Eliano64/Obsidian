...
- 发送方用户代理 **主动** 将邮件推送到发送方邮件服务器。
- 发送方邮件服务器  **主动** 将邮件推送到接收方邮件服务器。
...
|---|---|---|---|
| 1 | （无） | `220` 服务就绪 | 建立 TCP 连接后服务器主动推送 |
| 2 | `HELO` 客户域名 | `250` OK | 客户表明身份 |
| 3 | `MAIL FROM: <发件人地址>` | `250` OK | 告知邮件来自谁 |
| 4 | `RCPT TO: <收件人地址>` | `250` OK | 告知邮件发给谁 |
| 5 | `DATA` | `354` 开始接收 | 客户准备发送邮件内容 |
| 6 | 邮件内容（含首部和主体） | （无） | 客户发送邮件正文 |
| 7 | `<CRLF>.<CRLF>`（结束符） | `250` OK | 客户标记邮件结束 |
| 8 | `QUIT` | `221` 关闭连接 | 客户请求释放 TCP 连接 |

# 邮件读取协议：POP3 与 IMAP4

邮件发送用 SMTP（推），邮件读取用 POP3 或 IMAP4（拉）。这里的**拉**是指读取动作由接收方用户代理主动发起：邮件先保存在接收方邮件服务器的邮箱中，客户端在用户查看、定时同步或手动刷新时向服务器请求邮件，再把邮件下载到本地或同步其状态。服务器不会仅因邮件到达就用 POP3 或 IMAP4 主动把完整邮件送进尚未请求的客户端。
