...
**拥塞**是指某个转发位置的分组到达速度持续超过其处理或发送速度，来不及转发的分组不断积压在队列中。短暂排队不等于拥塞；只有输入长期压过输出，队列持续增长，才说明这里发生了拥塞。缓冲区有限，因此继续积压最终会造成排队时延增大和分组丢失。

若随着通信子网负载的增加，吞吐量反而降低，我们便可以认为网络中发生了拥塞。
